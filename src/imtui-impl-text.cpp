...
void ScanLine(int x1, int y1, int x2, int y2, int ymax,
...
void drawTriangle(ImVec2 p0, ImVec2 p1, ImVec2 p2, unsigned char col,
                  ImTui::TScreen *screen) {
  int ymin =
      std::min(std::min(std::min((float)screen->size(), p0.y), p1.y), p2.y);
...
inline ImTui::TColor rgbToAnsi256(ImU32 col, bool doAlpha) {
  ...
  ImTui::TColor res = 16 + (36 * std::round((float(r) / 255.0f) * 5.0f)) +
      ...
      const ImDrawCmd *pcmd = &cmd_list->CmdBuffer[cmd_i];
      {
        ...
        if (clip_rect.x < fb_width && clip_rect.y < fb_height &&
          ...
          for (unsigned int i = 0; i < pcmd->ElemCount; i += 3) {
            ...
            pos0.x =
                std::max(std::min(float(clip_rect.z - 1), pos0.x), clip_rect.x);
            pos1.x =
                std::max(std::min(float(clip_rect.z - 1), pos1.x), clip_rect.x);
            pos2.x =
                std::max(std::min(float(clip_rect.z - 1), pos2.x), clip_rect.x);
            pos0.y =
                std::max(std::min(float(clip_rect.w - 1), pos0.y), clip_rect.y);
            pos1.y =
                std::max(std::min(float(clip_rect.w - 1), pos1.y), clip_rect.y);
            pos2.y =
                std::max(std::min(float(clip_rect.w - 1), pos2.y), clip_rect.y);
            ...
            if (uv0.x != uv1.x || uv0.x != uv2.x || uv1.x != uv2.x ||
              ...
              float x =
                  ((pos0.x + pos1.x + pos2.x + ppos0.x + ppos1.x + ppos2.x) /
                   6.0f);
              float y =
                  ((pos0.y + pos1.y + pos2.y + ppos0.y + ppos1.y + ppos2.y) /
                   6.0f) +
                  0.5f;

              if (std::fabs(y - lastCharY) < 0.5f &&
                  std::fabs(x - lastCharX) < 0.5f) {
                x = lastCharX + 1.0f;
                y = lastCharY;
              }
              ...
              int xx = (x) + 1;
              int yy = (y) + 0;
              if (xx < clip_rect.x || xx >= clip_rect.z || yy < clip_rect.y ||
                  yy >= clip_rect.w) {
              } else {
