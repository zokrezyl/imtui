...
bool render_frame() {
  ...
  for (int windowId = 0; windowId < stateUI.nWindows; ++windowId) {
    ...
    std::string title = "[Y] Hacker News (" + UI::kContentStr[window.content] +
    ...
    if (window.showComments == false) {
      ...
      for (int i = 0; i < nShow; ++i) {
        ...
        if (std::holds_alternative<HN::Story>(item.data)) {
          ...
          if (windowId == stateUI.hoveredWindowId &&
            ...
            ImGui::GetWindowDrawList()->AddRectFilled(p0, p1,
                                                      ImGui::GetColorU32(col0));

            if (ImGui::IsKeyPressed('o', false) ||
                ImGui::IsKeyPressed('O', false)) {
          ...
          if (stateUI.storyListMode != UI::StoryListMode::Micro) {
            ImGui::TextDisabled("    %d points by %s %s ago | %d comments",
                                story.score, story.by.c_str(),
                                stateHN.timeSince(story.time).c_str(),
                                story.descendants);
            isHovered |= ImGui::IsItemHovered();
          }
          ...
          if (stateUI.storyListMode != UI::StoryListMode::Micro) {
            ImGui::TextDisabled("    %d points by %s %s ago", job.score,
                                job.by.c_str(),
                                stateHN.timeSince(job.time).c_str());
            isHovered |= ImGui::IsItemHovered();
          }
        }
        ...
        if (ImGui::GetCursorScreenPos().y + 3 > ImGui::GetWindowSize().y) {
          ...
          if ((i == window.maxStories - 1) &&
              (ImGui::GetCursorScreenPos().y + 2 < ImGui::GetWindowSize().y)) {
            ++window.maxStories;
          }
        }
      }
      ...
      if (windowId == stateUI.hoveredWindowId) {
        if (ImGui::IsMouseDoubleClicked(0) ||
          ...
          if (stateUI.showHelpModal == false &&
              window.hoveredStoryId < (int)storyIds.size()) {
        ...
        if (ImGui::IsKeyPressed('k', true) ||
            ImGui::IsKeyPressed(ImGui::GetIO().KeyMap[ImGuiKey_UpArrow],
                                true)) {
          window.hoveredStoryId = std::max(0, window.hoveredStoryId - 1);
        }

        if (ImGui::IsKeyPressed('j', true) ||
            ImGui::IsKeyPressed(ImGui::GetIO().KeyMap[ImGuiKey_DownArrow],
                                true)) {
          window.hoveredStoryId =
              std::max(0, std::min(nShow - 1, window.hoveredStoryId + 1));
        }
        ...
        if (ImGui::IsKeyPressed(ImGui::GetIO().KeyMap[ImGuiKey_Tab])) {
          stateUI.windows[stateUI.hoveredWindowId].content =
              (UI::WindowContent)(
                  (int(stateUI.windows[stateUI.hoveredWindowId].content) + 1) %
                  int(UI::WindowContent::Count));
        }
      }
    } else {
      if (std::holds_alternative<HN::Story>(
              items.at(window.selectedStoryId).data) == false) {
        window.showComments = false;
      } else {
        const auto &story =
            std::get<HN::Story>(items.at(window.selectedStoryId).data);

        toRefresh.push_back(story.id);

        ImGui::Text("%s", story.title.c_str());
        ImGui::TextDisabled("%d points by %s %s ago | %d comments", story.score,
                            story.by.c_str(),
                            stateHN.timeSince(story.time).c_str(),
                            story.descendants);
        ...
        std::function<void(const HN::ItemIds &commentIds, int indent)>
            renderComments;
        renderComments = [&](const HN::ItemIds &commentIds, int indent) {
          ...
          for (int i = 0; i < nComments; ++i) {
            ...
            if (items.find(id) == items.end() ||
                std::holds_alternative<HN::Comment>(items.at(id).data) ==
                    false) {
              continue;
            }
            ...
            char header[128];
            snprintf(header, 128, "%*s %s %s ago [%s]", indent, "",
                     comment.by.c_str(),
                     stateHN.timeSince(comment.time).c_str(),
                     stateUI.collapsed[id] ? "+" : "-");

            if (windowId == stateUI.hoveredWindowId &&
                curCommentId == window.hoveredCommentId) {
            ...
            if (windowId == stateUI.hoveredWindowId &&
                curCommentId == window.hoveredCommentId) {
              if (ImGui::GetCursorScreenPos().y >
                  ImGui::GetWindowSize().y + 4) {
        ...
        ImGui::BeginChild("##comments", ImVec2(0, 0), false,
        ...
        if (windowId == stateUI.hoveredWindowId) {
          ...
          if (ImGui::IsKeyPressed('j', true) ||
              ImGui::IsKeyPressed(ImGui::GetIO().KeyMap[ImGuiKey_DownArrow],
                                  true)) {
            window.hoveredCommentId =
                std::min(curCommentId - 1, window.hoveredCommentId + 1);
          }
          ...
          if (ImGui::IsKeyPressed('o', false) ||
              ImGui::IsKeyPressed('O', false)) {
            openInBrowser(
                (std::string("https://news.ycombinator.com/item?id=") +
                 std::to_string(story.id))
                    .c_str());
          }

          if (ImGui::IsMouseClicked(1) || ImGui::IsMouseClicked(2) ||
              ImGui::IsKeyPressed('q', false) ||
              ImGui::IsKeyPressed('b', false) ||
              ImGui::IsKeyPressed(ImGui::GetIO().KeyMap[ImGuiKey_Backspace],
                                  false)) {
            window.showComments = false;
          }
        }
        ...
        window.hoveredCommentId =
            std::min(curCommentId - 1, window.hoveredCommentId);
      }
    }
  ...
  if (ImGui::IsKeyPressed(ImGui::GetIO().KeyMap[ImGuiKey_RightArrow], true)) {
    stateUI.hoveredWindowId =
        std::min(stateUI.nWindows - 1, stateUI.hoveredWindowId + 1);
  }

  if (stateUI.showStatusWindow) {
    {
      auto wSize = ImGui::GetIO().DisplaySize;
      ImGui::SetNextWindowPos(ImVec2(0, wSize.y - stateUI.statusWindowHeight),
                              ImGuiCond_Always);
      ImGui::SetNextWindowSize(ImVec2(wSize.x, stateUI.statusWindowHeight),
                               ImGuiCond_Always);
    }
    snprintf(stateUI.statusWindowHeader, 512, "Status | Story List Mode : %d |",
             (int)(stateUI.storyListMode));
    ImGui::Begin(stateUI.statusWindowHeader, nullptr,
                 ImGuiWindowFlags_NoCollapse | ImGuiWindowFlags_NoResize |
                     ImGuiWindowFlags_NoMove);
    ImGui::Text(" API requests     : %d / %d B (next update in %d s)",
                stateHN.nFetches, (int)stateHN.totalBytesDownloaded,
                stateHN.nextUpdate);
  ...
  if (ImGui::IsKeyPressed('v', false)) {
    stateUI.storyListMode = (UI::StoryListMode)(
        ((int)(stateUI.storyListMode) + 1) % ((int)(UI::StoryListMode::COUNT)));
  }
  ...
  stateUI.hoveredWindowId =
      std::min(stateUI.nWindows - 1, stateUI.hoveredWindowId);

  if (stateUI.showHelpWelcome ||
      (stateUI.showHelpModal == false &&
       (ImGui::IsKeyReleased('h') || ImGui::IsKeyReleased('H')))) {
  ...
  if (ImGui::BeginPopupModal("HNTerm v0.3###Help", NULL,
...
int main([[maybe_unused]] int argc, [[maybe_unused]] char **argv) {
  ...
  int mouseSupport =
      argm.find("--mouse") != argm.end() || argm.find("m") != argm.end();
  ...
  g_screen = ImTui_ImplXcurses_Init(mouseSupport != 0, 60.0, 3.0);
#endif
  ImTui_ImplText_Init();
