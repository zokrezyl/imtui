...
int32_t tGet() { return time(nullptr); }
...
struct UI {
  enum EStyle { EStyle_Default, EStyle_Dark };
  ...
  void renderSeparator(const char *prefix, const char *ch, const char *suffix,
                       int n, bool disabled);
  ...
  bool renderMessages(const std::vector<MessageWithReplies> &messages,
...
void UI::renderSeparator(const char *prefix, const char *ch, const char *suffix,
                         int n, bool disabled) {
  static char buf[256];
  if (n > 255)
    n = 255;
...
bool UI::renderChannel(const Channel &channel, int width, bool selected) {
  ...
  if (ImGui::IsItemHovered() || selected || channel.hasUnread ||
      channel.mentions > 0) {
...
bool UI::renderMessages(const std::vector<MessageWithReplies> &messages,
                        int width, bool isThread) {
  int32_t lastUid = -1;
  std::string lastDate = "";
  ...
  for (int i = 0; i < (int)messages.size(); i++) {
    ...
    if (lastUid != uid) {
      ...
      ImGui::SameLine();
      ImGui::TextColored(colors.messageTime, "%s",
                         tToTime(message.msg.t_s).c_str());
    }
    ...
    if (message.replies.size() > 0) {
      ...
      ImGui::TextColored(colors.messageReplies, "%d replies",
                         (int)message.replies.size());
    }

    const auto p1 = ImVec2{ImGui::GetCursorScreenPos().x + width,
    ...
    if (ImGui::IsWindowHovered() && ImGui::IsMouseHoveringRect(p0, p1)) {
      ...
      drawList->AddRectFilled(
          {p0.x, p0.y + 1}, {p1.x, p1.y - 1},
          ImGui::ColorConvertFloat4ToU32(colors.messageHovered));
        ...
        if (ImGui::IsItemHovered()) {
          ...
          for (int i = 0; i < message.msg.reactUp; i++) {
            ...
            ImGui::TextColored(colors.messageUser, "%s%s", username.c_str(),
                               i == message.msg.reactUp - 1   ? ""
                               : i == message.msg.reactUp - 2 ? " and"
                                                              : ",");
      ...
      if (message.msg.reactDown > 0) {
        ...
        if (ImGui::IsItemHovered()) {
          ...
          for (int i = 0; i < message.msg.reactDown; i++) {
            const auto &username = users[i % users.size()].username;
            ImGui::TextColored(colors.messageUser, "%s%s", username.c_str(),
                               i == message.msg.reactDown - 1   ? ""
                               : i == message.msg.reactDown - 2 ? " and"
                                                                : ",");
            if (w += username.size(); w < 20) {
              ImGui::SameLine();
            } else {
      ...
      if (ImGui::IsItemHovered()) {
        ...
        ImGui::SetNextWindowPos(
            {ImGui::GetIO().MousePos.x - 10, ImGui::GetIO().MousePos.y - 2});
      ...
      if (!isThread) {
        ...
        if (ImGui::IsItemHovered()) {
          ...
          ImGui::SetNextWindowPos(
              {ImGui::GetIO().MousePos.x - 16, ImGui::GetIO().MousePos.y - 2});
...
bool render_frame() {
...
#else
  bool isActive = false;
  isActive |= ImTui_ImplXcurses_NewFrame();
#endif
  ...
  ImGui::SetNextWindowSize(ImVec2(g_screen->nx, g_screen->ny),
                           ImGuiCond_Always);
  ...
  // About Slack
  {
    ...
    ImGui::SetNextWindowPos(
        ImVec2(g_screen->nx / 2.0f - g_ui.aboutWindowW / 2.0f,
               g_screen->ny / 3.0f),
        ImGuiCond_Always);
    ...
    if (ImGui::BeginPopupModal("About Slack", NULL,
...

      {
        ...
        {
          const char *txt = "#";
          ImGui::SetCursorScreenPos({p0.x + g_ui.aboutWindowW / 2.0f -
                                         ImGui::CalcTextSize(txt).x / 2.0f,
                                     p0.y});
          auto col = g_ui.colors.mainWindowTitleFG;
          const auto t = ImGui::GetTime();
          const int it = (int(t / 0.25f)) % 5;
          if (it == 0)
            col = toVec4(255, 255, 255);
          if (it == 1)
            col = toVec4(128, 128, 255);
          if (it == 2)
            col = toVec4(0, 255, 0);
          if (it == 3)
            col = toVec4(255, 0, 0);
          if (it == 4)
            col = toVec4(255, 255, 0);
  ...
  // left panel
  {
...

    {
      ...
      ImGui::SetNextWindowSize(ImVec2(g_ui.workspaceInfoW, 0),
                               ImGuiCond_Always);
      if (ImGui::BeginPopup("Workspace Info")) {
        ImGui::PushTextWrapPos(ImGui::GetCursorPos().x + g_ui.workspaceInfoW -
                               4);
        ...
        g_ui.renderButton("Settings & administration", g_ui.workspaceInfoW,
                          false);
    ...
    if (g_ui.showChannels) {
      ...
      for (int i = 0; i < (int)g_ui.channels.size(); ++i) {
        ...
        if (g_ui.renderChannel(channel, g_ui.leftPanelW,
                               i == g_ui.selectedChannel)) {
    ...
    if (g_ui.renderList("Direct messages", g_ui.leftPanelW,
                        g_ui.showDirectMessages)) {
    ...
    ImGui::TextDisabled("%g %g %d %d", ImGui::GetIO().MousePos.x,
                        ImGui::GetIO().MousePos.y, ImGui::GetIO().MouseDown[0],
    ...
    if (ImGui::IsWindowHovered(ImGuiHoveredFlags_RootWindow) &&
        abs(ImGui::GetIO().MousePos.x - g_ui.leftPanelW) < 2) {
  ...
  if (g_ui.isHoveringLeftPanelResize) {
    ...
    drawList->AddRect(ImVec2(g_ui.leftPanelW + 0.5, 0),
                      ImVec2(g_ui.leftPanelW + 1.1, g_screen->ny),
  ...
  // main window
  {
    ...
    ImGui::BeginChild("main", ImVec2(g_ui.mainWindowW, g_ui.mainWindowH), true,
    ...
    if (g_ui.selectedChannel >= 0) {
      ...
      ImGui::BeginChild(
          "messages", ImVec2(g_ui.mainWindowW - 2, g_ui.messagesWindowH), true);
      ...
      if (ImGui::BeginPopupModal("Channel Info", NULL,
        ...
        {
          const auto p0 = ImGui::GetCursorScreenPos();
          ImGui::TextColored(g_ui.colors.mainWindowTitleFG, "# %s",
                             channel.label.c_str());
        ...
        if (ImGui::BeginTabBar("##tabbar", ImGuiTabBarFlags_None)) {
          ...
          {
            static char buf[16];
            snprintf(buf, sizeof(buf), "Members %d",
                     (int)channel.members.size());
            ImGui::PushStyleColor(ImGuiCol_Text, g_ui.colors.mainWindowTitleFG);
    ...
    if (g_ui.selectedUser >= 0) {
      ...
      ImGui::BeginChild(
          "messages", ImVec2(g_ui.mainWindowW - 1, g_ui.messagesWindowH), true);
...

    {
      ImGui::BeginChild(
          "input",
          ImVec2(g_ui.mainWindowW - 1, g_ui.mainWindowH - g_ui.messagesWindowH),
          true);
...

      {
        ...
        // toolbar
        {
          ...
          if (ImGui::IsItemHovered()) {
            ...
            ImGui::BeginTooltip();
            ImGui::Text("%s", "");
            ImGui::TextColored(g_ui.colors.inputTooltip,
        ...
        // send button
        {
          ...
          ImGui::PushStyleColor(ImGuiCol_ButtonHovered,
                                g_ui.colors.inputSendBGHovered);
          ImGui::PushStyleColor(ImGuiCol_ButtonActive,
                                g_ui.colors.inputSendBGActive);
        ...
        {
          ImGui::SetNextWindowPos(
              {ImGui::GetIO().MousePos.x, ImGui::GetIO().MousePos.y - 14},
              ImGuiCond_Appearing);
          if (ImGui::BeginPopup("@", ImGuiWindowFlags_NoScrollbar)) {
            ...
            for (int i = 0; i < (int)users.size(); ++i) {
              const auto &user = users[i];
              if (g_ui.renderUser(user, g_ui.mentionPanelW,
                                  i == g_ui.selectedUser)) {
                strcat(input, "@");
      ...
      if (doSend) {
        if (g_ui.selectedChannel > 0) {
          g_ui.channels[g_ui.selectedChannel].messages.push_back({{
                                                                      tGet(),
                                                                      0,
                                                                      input,
                                                                      0,
        ...
        if (g_ui.selectedUser > 0) {
          g_ui.users[g_ui.selectedUser].messages.push_back({{
                                                                tGet(),
                                                                0,
                                                                input,
                                                                0,
    ...
    if (ImGui::IsWindowHovered() &&
        abs(ImGui::GetIO().MousePos.x - g_ui.leftPanelW - g_ui.mainWindowW) <
            2) {
  ...
  if (g_ui.isHoveringThreadPanelResize) {
    ...
    drawList->AddRect(
        ImVec2(g_ui.leftPanelW + g_ui.mainWindowW + 0.5, 0),
        ImVec2(g_ui.leftPanelW + g_ui.mainWindowW + 1.1, g_screen->ny),
        ImGui::GetColorU32(ImGuiCol_Border));
  }
  ...
  if (g_ui.showThreadPanel) {
    ...
    ImGui::PushStyleColor(ImGuiCol_Text, g_ui.colors.mainWindowTextFG);
    ImGui::PushStyleColor(ImGuiCol_ChildBg, g_ui.colors.mainWindowBG);
    ImGui::SetNextWindowPos(ImVec2(g_ui.leftPanelW + g_ui.mainWindowW + 1, 0),
                            ImGuiCond_Always);

    ImGui::BeginChild(
        "thread", ImVec2(g_ui.threadPanelW, g_ui.threadPanelH), true,
    ...
    if (g_ui.threadChannel >= 0) {
      ...
      ImGui::BeginChild("messages",
                        ImVec2(g_ui.threadPanelW - 1, g_ui.threadPanelH - 6),
                        true);

      g_ui.renderMessages({{channel.messages[g_ui.threadMessage].msg, {}}},
                          g_ui.threadPanelW, true);

      if (int nreplies = channel.messages[g_ui.threadMessage].replies.size();
          nreplies > 0) {
        ImGui::Text("%s", "");
        {
          static char buf[32];
      ...
      g_ui.renderMessages(channel.messages[g_ui.threadMessage].replies,
                          g_ui.threadPanelW, true);
...

      {
        ...
        // send button
        {
          ...
          ImGui::SetCursorScreenPos(
              ImVec2(p0.x + g_ui.threadPanelW - 12, p0.y));
          ImGui::PushStyleColor(ImGuiCol_Text, g_ui.colors.inputSendFG);
          ImGui::PushStyleColor(ImGuiCol_Button, g_ui.colors.inputSendBG);
          ImGui::PushStyleColor(ImGuiCol_ButtonHovered,
                                g_ui.colors.inputSendBGHovered);
      ...
      if (doSend) {
        g_ui.channels[g_ui.threadChannel]
            .messages[g_ui.threadMessage]
            .replies.push_back({{
                                    tGet(),
                                    0,
                                    input,
  ...
  if (g_ui.showStyleEditor) {
...

    {
      ImGui::BeginChild(
          "##Colors",
          ImVec2(0, ImGui::GetWindowSize().y - ImGui::GetCursorPosY() - 3),
          true);
      ...
      changed |= ImGui::ColorEdit3("itemBGHovered",
                                   (float *)&g_ui.colors.itemBGHovered);
      changed |=
          ImGui::ColorEdit3("itemBGActive", (float *)&g_ui.colors.itemBGActive);

      changed |=
          ImGui::ColorEdit3("leftPanelBG", (float *)&g_ui.colors.leftPanelBG);
      changed |= ImGui::ColorEdit3("leftPanelTextFG",
                                   (float *)&g_ui.colors.leftPanelTextFG);
      changed |= ImGui::ColorEdit3("leftPanelTitleFG",
                                   (float *)&g_ui.colors.leftPanelTitleFG);

      changed |=
          ImGui::ColorEdit3("mainWindowBG", (float *)&g_ui.colors.mainWindowBG);
      changed |= ImGui::ColorEdit3("mainWindowTextFG",
                                   (float *)&g_ui.colors.mainWindowTextFG);
      changed |= ImGui::ColorEdit3("mainWindowTitleFG",
                                   (float *)&g_ui.colors.mainWindowTitleFG);

      changed |= ImGui::ColorEdit3("popupBG", (float *)&g_ui.colors.popupBG);

      changed |=
          ImGui::ColorEdit3("listHeaderFG", (float *)&g_ui.colors.listHeaderFG);

      changed |= ImGui::ColorEdit3("channelSelectedBG",
                                   (float *)&g_ui.colors.channelSelectedBG);
      changed |= ImGui::ColorEdit3("channelActiveFG",
                                   (float *)&g_ui.colors.channelActiveFG);
      changed |= ImGui::ColorEdit3("channelMentionsFG",
                                   (float *)&g_ui.colors.channelMentionsFG);
      changed |= ImGui::ColorEdit3("channelMentionsBG",
                                   (float *)&g_ui.colors.channelMentionsBG);

      changed |=
          ImGui::ColorEdit3("userActiveFG", (float *)&g_ui.colors.userActiveFG);

      changed |=
          ImGui::ColorEdit3("messageUser", (float *)&g_ui.colors.messageUser);
      changed |=
          ImGui::ColorEdit3("messageTime", (float *)&g_ui.colors.messageTime);
      changed |= ImGui::ColorEdit3("messageHovered",
                                   (float *)&g_ui.colors.messageHovered);
      changed |=
          ImGui::ColorEdit3("messageReact", (float *)&g_ui.colors.messageReact);
      changed |= ImGui::ColorEdit3("messageReplies",
                                   (float *)&g_ui.colors.messageReplies);

      changed |= ImGui::ColorEdit3("inputBG", (float *)&g_ui.colors.inputBG);
      changed |=
          ImGui::ColorEdit3("inputSendBG", (float *)&g_ui.colors.inputSendBG);
      changed |=
          ImGui::ColorEdit3("inputSendFG", (float *)&g_ui.colors.inputSendFG);
      changed |= ImGui::ColorEdit3("inputSendBGHovered",
                                   (float *)&g_ui.colors.inputSendBGHovered);
      changed |= ImGui::ColorEdit3("inputSendBGActive",
                                   (float *)&g_ui.colors.inputSendBGActive);
      changed |=
          ImGui::ColorEdit3("inputTooltip", (float *)&g_ui.colors.inputTooltip);

      changed |= ImGui::ColorEdit3("buttonActiveFG",
                                   (float *)&g_ui.colors.buttonActiveFG);

      changed |= ImGui::ColorEdit3("indicatorOnline",
                                   (float *)&g_ui.colors.indicatorOnline);
      changed |= ImGui::ColorEdit3("indicatorOffline",
                                   (float *)&g_ui.colors.indicatorOffline);
    ...
    if (ImGui::Button(" To File ")) {
      ...
      auto to_s = [](const ImVec4 &col) {
        return std::to_string(col.x) + ", " + std::to_string(col.y) + ", " +
      ...
      res +=
          ".mainWindowBG      = { " + to_s(g_ui.colors.mainWindowBG) + " },\n";
      ...
      res +=
          ".messageHovered = { " + to_s(g_ui.colors.messageHovered) + " },\n";
      res += ".messageReact   = { " + to_s(g_ui.colors.messageReact) + " },\n";
      res +=
          ".messageReplies = { " + to_s(g_ui.colors.messageReplies) + " },\n";

      res += ".inputBG            = { " + to_s(g_ui.colors.inputBG) + " },\n";
      res +=
          ".inputSendBG        = { " + to_s(g_ui.colors.inputSendBG) + " },\n";
      res +=
          ".inputSendFG        = { " + to_s(g_ui.colors.inputSendFG) + " },\n";
      res += ".inputSendBGHovered = { " + to_s(g_ui.colors.inputSendBGHovered) +
             " },\n";
      res += ".inputSendBGActive  = { " + to_s(g_ui.colors.inputSendBGActive) +
             " },\n";
      res +=
          ".inputTooltip       = { " + to_s(g_ui.colors.inputTooltip) + " },\n";

      res +=
          ".buttonActiveFG = { " + to_s(g_ui.colors.buttonActiveFG) + " },\n";
...
#ifndef __EMSCRIPTEN__
  ImTui_ImplXcurses_DrawScreen(isActive);
#endif
...
int main() {
    ...
    for (auto &channel : g_ui.channels) {
      ...
      for (int i = 0; i < nMessages; ++i) {
        const bool isSameUser =
            (logs[logId - 1].username == logs[logId].username);

        int uid =
            lastUid == -1 || !isSameUser ? rand() % g_ui.users.size() : lastUid;
  ...
  g_screen = ImTui_ImplXcurses_Init(true, 60.0, 3.0);
#endif
